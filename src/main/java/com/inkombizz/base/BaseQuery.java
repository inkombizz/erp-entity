...
public abstract class BaseQuery extends Session {
  ...
  public PaginatedBase pagingResult(String count, String select, int from, int to) {
    long row = countResult(count);
    session = em.unwrap(org.hibernate.Session.class);
    List<Map<String, Object>> res = session.createNativeQuery(select)
            .setFirstResult(from)
            .setMaxResults(to)
            .setResultTransformer(AliasToEntityMapResultTransformer.INSTANCE)
            .list();

    return new PaginatedBase(res, "data success", from, row, (row / to) + 1);
  }
